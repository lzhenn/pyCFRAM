      ...
      integer,parameter:: xt=288,yt=192,z1=17,z18=18,nn=56
      real             :: plev(1:z1),ch4(1:nn),ch4ts
...
! 设置 ch4 浓度，从 0.7 到 2.5
      do nnn = 1, nn
         ch4(nnn) = 0.7 + real(nnn) * 0.05  
      enddo
...
C     NCLFORTSTART
     ...
     & cld_amt,ch4mass,ice_wat,liq_wat,solar_in,
...
C       from AR4 data, Uses huss ctl run and hus ch4 run, and surface pressure ch4 run
      include 'para.file'
...
c      real tas(IX,IY),huss(IX,IY),rlus(IX,IY)
      ...
      common /umcon/ umch4, umn2o, umco2
...
*
      ...
      write(mm_ch,'(I0)') nint(ch4mass*100)  ! 将 ch4 浓度值转换为整数字符串，例如 364
      print*,'Processing ch4 concentration: ', mm_ch
*-------------------------------------------------------------------------------
*     OUTPUT
*
       open ( unit = 22, file =
     & './ch4_radranc_'//Trim(AdjustL(mm_ch))//'.grd',
     & form='unformatted', access='direct',recl= ix*iy )
       open ( unit = 32, file =
     & './ch4_radsfc_ranc_'//Trim(AdjustL(mm_ch))//'.grd',
     & form='unformatted', access = 'direct',recl = ix*iy )

       open ( unit = 51, file =
     & './ch4_input_'//Trim(AdjustL(mm_ch))//'.dat',
     & form='unformatted', access = 'direct', recl = ix*iy )
       open ( unit = 52, file =
