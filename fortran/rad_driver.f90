subroutine rad_driver(nlayer,iaer, icld,  co2, ch4, n2o, ps, ts, zenith,&
    albedo_sw,albedo_lw, plev, t, q, o3, cldfrac_cfram, cldlwc, cldiwc, tauaer_sw_cfram,& 
    ssaaer_sw_cfram, asmaer_sw_cfram, tauaer_lw_cfram,&
    fds, fus, htr_sw, fdl, ful, htr_lw, fd, fu, htr, rad_base, lw_base, sw_base)
    ...
    real(kind=rb), intent(in)    :: zenith       ! cosine of solar zenith angle
    real(kind=rb), intent(in)    :: albedo_sw    ! surface albedo for sw
    ...
    real(kind=rb), intent(in)    :: tauaer_sw_cfram(nlayer,jpband) !aerosol optical depth
    real(kind=rb), intent(in)    :: ssaaer_sw_cfram(nlayer,jpband) !aerosol single scattering albedo
    real(kind=rb), intent(in)    :: asmaer_sw_cfram(nlayer,jpband) !aerosol asymmetry factor
    real(kind=rb), intent(in)    :: tauaer_lw_cfram(nlayer,nbndlw)    ! optical depth for longwave each band
    real(kind=rb), intent(out)   :: fds(nlayer+1)
    real(kind=rb), intent(out)   :: fus(nlayer+1)
    real(kind=rb), intent(out)   :: htr_sw(nlayer)
    real(kind=rb), intent(out)   :: fdl(nlayer+1)
    real(kind=rb), intent(out)   :: ful(nlayer+1)
    real(kind=rb), intent(out)   :: htr_lw(nlayer)
    real(kind=rb), intent(out)   :: fd(nlayer+1)
    real(kind=rb), intent(out)   :: fu(nlayer+1)
    real(kind=rb), intent(out)   :: htr(nlayer)
    real(kind=rb), intent(out)   :: rad_base(nlayer)
    real(kind=rb), intent(out)   :: lw_base(nlayer)
    real(kind=rb), intent(out)   :: sw_base(nlayer)
   ...
   ! column density for broadening gases(molecules/cm**2)
    ...
    real(kind=rb) :: semiss_sw(jpband) ! sw surface emissivity for each band
   ...
   ! column density for broadening gases(molecules/cm**2)
    ...
    real(kind=rb) :: tauaer_sw(mxlay,jpband)
    real(kind=rb) :: ssaaer_sw(mxlay,jpband)
    real(kind=rb) :: asmaer_sw(mxlay,jpband)
    ...
    real(kind=rb) :: uflxsum_sw(0:mxlay) ! sw upward flux (W/m**2)
    real(kind=rb) :: dflxsum_sw(0:mxlay) ! sw downward flux (W/m**2)
    real(kind=rb) :: fnetsum_sw(0:mxlay) ! sw net flux (W/m**2)
    real(kind=rb) :: htrsum_sw(0:mxlay)  ! sw heating rate (K/day)
...
!------------------------------------------------------------------------------------
   ...
   if (iaer .ne. 0) then
       do ilayer = 1,nlayer
           ...
           tauaer_sw(nlayer-ilayer+1,:) = tauaer_sw_cfram(ilayer,:)
           ssaaer_sw(nlayer-ilayer+1,:) = ssaaer_sw_cfram(ilayer,:)
           asmaer_sw(nlayer-ilayer+1,:) = asmaer_sw_cfram(ilayer,:)
       end do
   end if
    ...
    call rrtmg_sw(iaer, icld, nlayer, pdp, pave, tave, pz, tz, tbound, semiss_sw,&
        zenith, coldry, wbrodl, wkl, cldfrac, ciwp, clwp, rei, rel, tauaer_sw, ssaaer_sw,&
        asmaer_sw, uflxsum_sw, dflxsum_sw, fnetsum_sw, htrsum_sw)

    do ilayer = 1,nlayer+1
        fds(ilayer) = dflxsum_sw(nlayer+1-ilayer)
        fus(ilayer) = uflxsum_sw(nlayer+1-ilayer)
    ...
    do ilayer = 1,nlayer
        ...
        htr_sw(ilayer) = htrsum_sw(nlayer-ilayer)
    end do
    ...
    htr               = htr_sw + htr_lw
    do ilayer = 1, nlayer
       sw_base(ilayer) = fds(ilayer) + fus(ilayer+1) - fds(ilayer+1) - fus(ilayer)
       lw_base(ilayer) = fdl(ilayer) + ful(ilayer+1) - fdl(ilayer+1) - ful(ilayer)
       rad_base(ilayer)= lw_base(ilayer) + sw_base(ilayer)
    end do
